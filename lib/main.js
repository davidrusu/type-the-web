...
let button = buttons.ActionButton({
    ...
    onClick: () => {
        ...
        let tabURL = tab.url; // we unpack the tab.url so that if the active tab changes we still can reference it
        let pair = [tab,tabURL];
        if (attachedTabs.indexOf(pair) !== -1) {
            return;
        }
        attachedTabs.push(pair);
        ...
        let removeAttachedTab = () => {
            let i = attachedTabs.indexOf(pair);
            if (i > -1) {
